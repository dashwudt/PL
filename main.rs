...
fn calc_hash(session_key: &String, val :&i64) -> String {
    ...
    if *val == 1 {
        ...
        for i in 0..session_key.len() {
            ...
            let mut cha =  std::char::from_u32(cha_ord ^ 43).unwrap();
            if !cha.is_digit(10) {
                num += cha as u32;
...
fn handle_client(mut stream: TcpStream) {
    ...
    while count_serv < max_count_serv {
        ...
        match stream.read(&mut msg) {
            Ok(size) => {
                let mut unwmsg = str::from_utf8(&msg).unwrap().to_string();
...
fn main() {
    println!("Введите port/ip:port");
    let mut error = true;
        while error {
            let mut ip_port = String::new();
            io::stdin()
                .read_line(&mut ip_port)
                .expect("Failed to read line");
            if ip_port.find(':') != None{
                let split_ip_port: Vec<&str> = ip_port.split(':').collect();
                if ip_port.find('.') != None{
                    let split_ip: Vec<&str> = split_ip_port[0].split('.').collect();
                    if split_ip.len()!=4{println!("U need specifically 3 dots");ip_port.clear(); error = true; continue;}
                    else{
                        if (split_ip[0].len()>3 || split_ip[0].len()<1) || (split_ip[1].len()>3 || split_ip[1].len()<1) || (split_ip[2].len()>3 || split_ip[2].len()<1) || (split_ip[3].len()>3 || split_ip[3].len()<1){
                            println!("Wrong ip size"); ip_port.clear(); error = true; continue;}
                        else{
                            println!("Client mode");                                                        //CLIENT CLIENT CLIENT
                ...
                }else{println!("U need ip with dots") ;ip_port.clear();error = true; continue;}
                    if split_ip_port.len() > 2{println!("Problems with port, too many : symbols");ip_port.clear();error = true; continue;
                    }else{
                        error = false;
                        if split_ip_port[1].len()< 4 {println!("Port should be at least 4 digit"); ip_port.clear(); error = true; continue;}
                    }
            }else{println!("Server mode!");                                                                                                                 //SERVER SERVER
                let port_u16 : u16 = ip_port[0..ip_port.len()-1].parse().unwrap();
